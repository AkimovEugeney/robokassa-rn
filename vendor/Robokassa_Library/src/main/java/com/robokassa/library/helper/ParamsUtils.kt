...
internal fun PaymentParams.payPostParams(isTest: Boolean): String = run {
    ...
    var result = ""
    var signature = ""
    val shpParams = this.order.collectShpParams()
...
private fun com.robokassa.library.params.OrderParams.collectShpParams(): List<Pair<String, String>> {
    val params = linkedMapOf<String, String>()
    params["shp_label"] = "sdk_android"
    ...
    extra.forEach { (rawKey, rawValue) ->
        ...
        val existingKey = params.keys.firstOrNull { it.equals(normalizedKey, ignoreCase = true) }
        if (existingKey != null) {
            params.remove(existingKey)
        }
    ...
    return params.entries
        .sortedWith(compareBy({ it.key.lowercase(Locale.ROOT) }, { it.key }))
